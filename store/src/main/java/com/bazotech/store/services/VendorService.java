...
public class VendorService {
	...
	/* Updating vendor-bio basic information */
	@Transactional
	...
	/* Updating Vendor Address Information */
	@Transactional
	public void updateVendorAddress(Long vendorID) {
		...
		vendorAddressList.stream().forEach(address ->{
			if(address.getBusinessAddressId().equals(1L)) {
				...
				address.setDescription("Updated Description for the address");
	...
	/* Removing a vendor address */
	@Transactional 
	public void removeExistingVendorAddress(Long vendorID) {
	
		var vendor = this.getVendorById(vendorID); 
		var iterator = vendor.getBusinessAddresses().iterator(); 
		...
		while (iterator.hasNext()) {
			...
			if (address.getBusinessAddressId().equals(2L)) { 
					iterator.remove(); // removes from list safely 
					address.setVendor(null); // break relationship 
			}
	...
	@Transactional
	public void addNewVendorAddress(Long vendorID) {
		
		var vendor = this.getVendorById(vendorID);
		
		var newAddress = BusinessAddress.builder()
										.country("Uganda")
										.city("Jinja")
										.street("New Street in Jinja")
										.region("Eastern Uganda")
										.merchant(null)
										.vendor(vendor)
										.addressType(BusinessAddress.AddressType.SECONDARY)
										.description("Newly added address in Jinja")
										.build();
		
		vendor.addBusinessAddress(newAddress);
	...
	public void deleteVendor(Long vendorID) {
		...
		logger.info("DeletedVendor:logger: Vendor with ID " + vendorID + " has been deleted.");
	}
